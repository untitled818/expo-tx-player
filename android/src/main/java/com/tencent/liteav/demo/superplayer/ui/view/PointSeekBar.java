 ...
 * <p>
 * In addition to the basic functions of a seekbar, it also has the ability to
...
public class PointSeekBar extends RelativeLayout {
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    // Solve the problem that the seekbar marker is ahead of the progress origin
    // before playback
    invalidate();
  ...
  protected void onDraw(Canvas canvas) {
    ...
    // Avoid the white space between the seekbar origin and the left end when
    // starting playback
  ...
  public void addPoint(PointParams pointParams, final int index) {
    ...
    mTcPointViewLayoutParams = new LayoutParams(mThumbDrawable.getIntrinsicWidth(),
        mThumbDrawable.getIntrinsicWidth());
    mTcPointViewLayoutParams.leftMargin = (int) ((pointParams.progress * 1.0f / mMaxProgress)
        * (mSeekBarRight - mSeekBarLeft));
  ...
  private boolean handleUpEvent(MotionEvent event) {
    // Need to use the x coordinate of the event, use mThumbLeft to make it
