...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        NSLog(@"📦 SuperPlayerKitBundle 路径: %@", bundlePath);

        NSBundle *bundle = [NSBundle bundleWithPath:bundlePath];
        NSString *imagePath = [bundle pathForResource:@"play" ofType:@"png"]; // 你要找的图片名
        NSLog(@"🔍 play 图片路径: %@", imagePath);
...
- (NSDictionary *)subtitlesConfig {
    return [self.subtitlesView subtitlesConfig];
}
#pragma mark - Action
